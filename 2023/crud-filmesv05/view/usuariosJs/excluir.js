...
function usuarioExcluirFetch(id){
    if(confirm('Confirma a exclusão ao usuário de id' +id+'?')){
        let usuario ={"id": id};
        let configMetodo ={
            method: "DELETE"
      ...
      // fetch enviando o id d usuario a ser excluido
      fetch("../controller/usuarioExcluir.php", configMetodo)
      .then(function(resposta){
        if(!resposta.ok===true){
            if(resposta.status===4001)
...
function fcSucessoExcluirUsuario(respostaJSON){
    ...
    setTimeout(function(){
        //Agora apenas aguarda 1,5 segundos para a mensagem ser exibidada e recarregar a lista
        limparSpan('#msg');
        usuarioListarFetch();
    },1500);
}
